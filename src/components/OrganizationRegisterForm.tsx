...
export default function OrganizationRegisterForm({ type }: OrganizationRegisterFormProps) {
  const { login } = useAuth();
  const isInstitution = type === 'institution';

  const title = isInstitution ? 'Institution Registration' : 'Company Registration';
  const roleName = isInstitution ? 'Institution' : 'Company';
  const defaultEmailPlaceholder = isInstitution ? 'admin@inst.rw' : 'admin@comp.rw';
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    login({
      name: formData.organizationName || `${roleName} Account`,
